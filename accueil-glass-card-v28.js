/* accueil-glass-card v28 — Confort thermique : double-curseur chauffe↔clim par zone (zone morte = écart) + anti-rebond
...
class AccueilGlassCard extends HTMLElement{
  ...
  _setPanelHtml(){
    ...
    const zoneRow=(zone,name,heatEnt,coolEnt,heatFb,coolFb)=>{
      ...
      const hp=pct(h),kp=pct(k);
      return `<div class="czRow">
        <div class="czHd"><span class="czName">${name}</span><span class="czGap" data-czgap="${zone}">confort ${Math.round(h)}–${Math.round(k)}°</span></div>
        <div class="czTrack">
    ...
    const eBm=c.bandeMorte;
    const mnB=this._attr(eBm,'min',0.5),mxB=this._attr(eBm,'max',4),spB=this._attr(eBm,'step',0.5),vB=this._f(eBm,2);
    const bmRow=`<div class="setRow setBmRow">
        <div class="setHd"><span class="setName">${this._ic('lines','seti')}Anti-rebond clim</span><span class="setVal setValBm" data-setval="bm">${vB.toFixed(1)}°</span></div>
        <input class="setSlider setBm" type="range" data-setslider="bm" data-dec="1" min="${mnB}" max="${mxB}" step="${spB}" value="${vB}">
    ...
    return `
      ...
      <div class="sheet setSheet">
        <div class="shHead">
          ${this._ic('cog','shBig')}
          <div class="shTit"><span class="shJ">Confort thermique</span><span class="shC">Chauffe ↔ clim par zone · l'écart = zone morte</span></div>
        ...
        <div class="setList">
          ...
          ${bmRow}
          <div class="setHint">Sous <b>chauffe</b>, la PAC chauffe ; au-dessus de <b>clim</b>, la climatisation démarre ; entre les deux = <b>zone morte</b> (rien ne tourne). L'anti-rebond évite que la clim cycle (coupe à clim − rebond). L'étage applique la même consigne aux 3 chambres.</div>
        </div>
      </div>`;
  }
  _autoPanelHtml(){
  ...
  _render(m){
    ...
    this.shadowRoot.querySelectorAll('[data-setslider]').forEach(el=>{
      const lab=this.shadowRoot.querySelector('[data-setval="bm"]');
      el.addEventListener('input',ev=>{ev.stopPropagation();if(lab)lab.textContent=parseFloat(el.value).toFixed(1)+'°';});
      el.addEventListener('change',ev=>{ev.stopPropagation();this._call('input_number','set_value',{entity_id:c.bandeMorte,value:parseFloat(el.value)});});
    });
    this.shadowRoot.querySelectorAll('.czH').forEach(inp=>{
      const sr=this.shadowRoot;
      ...
      const heatInp=sr.querySelector(`.czH[data-zone="${zone}"][data-kind="heat"]`);
      const coolInp=sr.querySelector(`.czH[data-zone="${zone}"][data-kind="cool"]`);
      const pc=v=>(((v-mn)/(mx-mn))*100);
      const redraw=()=>{
        let h=parseFloat(heatInp.value),k=parseFloat(coolInp.value);
        if(h>k-1){if(kind==='heat'){h=k-1;heatInp.value=h;}else{k=h+1;coolInp.value=k;}}
        const hf=sr.querySelector(`[data-czheatfill="${zone}"]`);if(hf)hf.style.width=pc(h)+'%';
        const cf=sr.querySelector(`[data-czcoolfill="${zone}"]`);if(cf){cf.style.left=pc(k)+'%';cf.style.width=(100-pc(k))+'%';}
        const vh=sr.querySelector(`[data-czvheat="${zone}"]`);if(vh)vh.textContent=Math.round(h);
        const vk=sr.querySelector(`[data-czvcool="${zone}"]`);if(vk)vk.textContent=Math.round(k);
        const g=sr.querySelector(`[data-czgap="${zone}"]`);if(g)g.textContent=`confort ${Math.round(h)}–${Math.round(k)}°`;
      };
      inp.addEventListener('input',ev=>{ev.stopPropagation();redraw();});
      inp.addEventListener('change',ev=>{
        ev.stopPropagation();redraw();
        const v=parseFloat(inp.value);
        if(kind==='heat')this._call('input_number','set_value',{entity_id:zone==='rdc'?c.heatRdc:c.heatEtage,value:v});
        else if(zone==='rdc')this._call('input_number','set_value',{entity_id:c.seuilRdc,value:v});
        else (Array.isArray(c.seuilEtage)?c.seuilEtage:[c.seuilEtage]).forEach(e=>this._call('input_number','set_value',{entity_id:e,value:v}));
      });
    });
