/* accueil-glass-card v27 — panneau Réglages (seuils RDC/Étage + zone morte, curseurs 15-30)
...
class AccueilGlassCard extends HTMLElement{
  ...
  _setPanelHtml(){
    ...
    const c=this._c;
    const eRdc=c.seuilRdc;
    const eEt=(Array.isArray(c.seuilEtage)?c.seuilEtage[0]:c.seuilEtage);
    const eBm=c.bandeMorte;
    const dec=sp=>(parseFloat(sp)<1?1:0);
    const tempRow=(id,name,ent,fallback)=>{
      const mn=this._attr(ent,'min',15),mx=this._attr(ent,'max',30),sp=this._attr(ent,'step',1);
      const v=this._f(ent,fallback);
      return `<div class="setRow">
        <div class="setHd"><span class="setName">${this._ic('snow','seti')}${name}</span><span class="setVal" data-setval="${id}">${v.toFixed(dec(sp))}°</span></div>
        <input class="setSlider" type="range" data-setslider="${id}" data-dec="${dec(sp)}" min="${mn}" max="${mx}" step="${sp}" value="${v}">
        <div class="setScale"><span>${Math.round(mn)}°</span><span>${Math.round(mx)}°</span></div>
      </div>`;
    };
    const mnB=this._attr(eBm,'min',0.5),mxB=this._attr(eBm,'max',4),spB=this._attr(eBm,'step',0.5),vB=this._f(eBm,2);
    const bmRow=`<div class="setRow">
        <div class="setHd"><span class="setName">${this._ic('lines','seti')}Zone morte</span><span class="setVal setValBm" data-setval="bm">${vB.toFixed(1)}°</span></div>
    ...
    return `
      ...
      <div class="sheet setSheet">
        <div class="shHead">
          ${this._ic('cog','shBig')}
          <div class="shTit"><span class="shJ">Réglages clim</span><span class="shC">Seuils de température & zone morte</span></div>
          <button class="shX" data-closeset="1">✕</button>
        </div>
        <div class="setList">
          ${tempRow('rdc','RDC · Salon',eRdc,24)}
          ${tempRow('etage','Étage · 3 chambres',eEt,22)}
          ${bmRow}
          <div class="setHint">La clim démarre au-dessus du seuil et se coupe à <b>seuil − zone morte</b>. Le curseur Étage applique la même consigne aux 3 chambres.</div>
        </div>
      </div>`;
  }
  _autoPanelHtml(){
  ...
  _render(m){
    ...
    this.shadowRoot.querySelectorAll('[data-setslider]').forEach(el=>{
      const id=el.getAttribute('data-setslider');
      const dec=parseInt(el.getAttribute('data-dec'),10)||0;
      const lab=this.shadowRoot.querySelector(`[data-setval="${id}"]`);
      el.addEventListener('input',ev=>{ev.stopPropagation();if(lab)lab.textContent=parseFloat(el.value).toFixed(dec)+'°';});
      el.addEventListener('change',ev=>{
        ev.stopPropagation();
        const v=parseFloat(el.value);
        if(id==='rdc')this._call('input_number','set_value',{entity_id:c.seuilRdc,value:v});
        else if(id==='etage')(Array.isArray(c.seuilEtage)?c.seuilEtage:[c.seuilEtage]).forEach(e=>this._call('input_number','set_value',{entity_id:e,value:v}));
        else if(id==='bm')this._call('input_number','set_value',{entity_id:c.bandeMorte,value:v});
      });
    });
