/* accueil-glass-card v31 — Confort ÉPURÉ : 2 poignées par défaut (chauffe · clim), coupe en pointillé passif, "Avancé" repliable (zone morte mini + anti-rebond)
   v30 — Confort : + réglage Zone morte mini (écart chauffe↔clim auto-respecté, pas de chevauchement)
   v29 — Confort : 3 repères/zone (chauffe · coupe pointillée · clim), coupe bornée à chauffe+1
   v28 — Confort thermique : double-curseur chauffe↔clim par zone (zone morte = écart) + anti-rebond
   v27 — panneau Réglages (seuils RDC/Étage + zone morte, curseurs 15-30)
   v26 — bouton Annuler sur l'alerte Quick Veto PAC
...
class AccueilGlassCard extends HTMLElement{
  constructor(){super();this.attachShadow({mode:'open'});this._last='';this._sheetDay=null;this._hf=null;this._hfTs=0;this._autoPanel=false;this._setPanel=false;}
  setConfig(cfg){
    this._c=Object.assign({
      ...
      seuilRdc:'input_number.clim_consigne_ete',
      seuilEtage:['input_number.clim_consigne_parents','input_number.clim_consigne_louise','input_number.clim_consigne_leandre'],
      bandeMorte:'input_number.clim_bande_morte',
      heatRdc:'input_number.confort_rdc_chaud',
      heatEtage:'input_number.confort_etage_chaud',
      zoneMorte:'input_number.clim_zone_morte_mini',
      climAutomations:[
        {g:'Pilotage',items:[
  ...
  _openSet(){this._setPanel=true;const m=this._model();this._lastStruct=this._structSig(m);this._render(m);}
  _closeSet(){this._setPanel=false;const m=this._model();this._lastStruct=this._structSig(m);this._render(m);}
  _attr(e,k,d){const st=this._h&&this._h.states[e];return st&&st.attributes&&st.attributes[k]!==undefined?st.attributes[k]:d;}
  _setPanelHtml(){
    if(!this._setPanel)return '';
    const c=this._c;
    const zoneRow=(zone,name,heatEnt,coolEnt,heatFb,coolFb)=>{
      const mn=this._attr(coolEnt,'min',15),mx=this._attr(coolEnt,'max',30),sp=this._attr(coolEnt,'step',1);
      const h=this._f(heatEnt,heatFb),k=this._f(coolEnt,coolFb);
      const rb=this._f(c.bandeMorte,2);
      const cut=Math.min(Math.max(k-rb,h+1),k-0.5);
      const pct=v=>(((v-mn)/(mx-mn))*100);
      const hp=pct(h),kp=pct(k),cp=pct(cut);
      return `<div class="czRow">
        <div class="czHd"><span class="czName">${name}</span><span class="czGap" data-czgap="${zone}">neutre ${Math.round(h)}–${Math.round(cut)}°</span></div>
        <div class="czTrack">
          <div class="czRail"></div>
          <div class="czHeatFill" data-czheatfill="${zone}" style="width:${hp}%"></div>
          <div class="czCoolBand" data-czband="${zone}" style="left:${cp}%;width:${kp-cp}%"></div>
          <div class="czCoolFill" data-czcoolfill="${zone}" style="left:${kp}%;width:${100-kp}%"></div>
          <div class="czCutLine" data-czcutline="${zone}" style="left:${cp}%"></div>
          <input class="czH czHeat" type="range" data-zone="${zone}" data-kind="heat" min="${mn}" max="${mx}" step="${sp}" value="${h}">
          <input class="czH czCool" type="range" data-zone="${zone}" data-kind="cool" min="${mn}" max="${mx}" step="${sp}" value="${k}">
        </div>
        <div class="czScale"><span>${Math.round(mn)}°</span><span>${Math.round(mx)}°</span></div>
        <div class="czVals">
          <span class="czVHeat">${this._ic('fire','czi')}chauffe <b data-czvheat="${zone}">${Math.round(h)}</b>°</span>
          <span class="czVCut">coupe clim <b data-czvcut="${zone}">${Math.round(cut)}</b>°</span>
          <span class="czVCool">${this._ic('snow','czi')}clim <b data-czvcool="${zone}">${Math.round(k)}</b>°</span>
        </div>
      </div>`;
    };
    const rb=this._f(c.bandeMorte,2);
    const mnB=this._attr(c.bandeMorte,'min',0.5),mxB=this._attr(c.bandeMorte,'max',4),spB=this._attr(c.bandeMorte,'step',0.5);
    return `
      <div class="veil" data-closeset="1"></div>
      <div class="sheet setSheet">
        <div class="shHead">
          ${this._ic('cog','shBig')}
          <div class="shTit"><span class="shJ">Confort thermique</span><span class="shC">Chauffe & clim par zone</span></div>
          <button class="shX" data-closeset="1">✕</button>
        </div>
        <div class="setList">
          ${zoneRow('rdc','RDC · Salon',c.heatRdc,c.seuilRdc,21,24)}
          ${zoneRow('etage','Étage · 3 chambres',c.heatEtage,(Array.isArray(c.seuilEtage)?c.seuilEtage[0]:c.seuilEtage),19,22)}
          <div class="setHint">Glisse <b>chauffe</b> (PAC en dessous) et <b>clim</b> (climatisation au-dessus). Entre les deux = zone neutre ; le pointillé montre où la clim s'arrête.</div>
          <details class="adv">
            <summary>${this._ic('cog','advi')}<span>Avancé</span><span class="advChev">›</span></summary>
            <div class="advBody">
              <div class="zmRow">
                <div class="zmLeft"><span class="zmName">Zone morte mini</span><span class="zmSub">écart minimum chauffe ↔ clim</span></div>
                <div class="zmStep"><button class="zmBtn" data-zm="-" aria-label="moins">−</button><span class="zmVal" data-zmval>${this._f(c.zoneMorte,2)}°</span><button class="zmBtn" data-zm="+" aria-label="plus">+</button></div>
              </div>
              <div class="setRow setBmRow">
                <div class="setHd"><span class="setName">${this._ic('lines','seti')}Anti-rebond clim</span><span class="setVal setValBm" data-setval="bm">${rb.toFixed(1)}°</span></div>
                <input class="setSlider setBm" type="range" data-setslider="bm" min="${mnB}" max="${mxB}" step="${spB}" value="${rb}">
                <div class="setScale"><span>${mnB}°</span><span>${mxB}°</span></div>
              </div>
              <div class="setHint">La <b>zone morte mini</b> empêche chauffe et clim de se chevaucher. L'<b>anti-rebond</b> = de combien la clim descend sous son seuil avant de couper (le pointillé). Communs aux deux zones.</div>
            </div>
          </details>
        </div>
      </div>`;
  ...
  _ic(n,s){
    const p={
      ...
      cog:'M19.4 13a7.8 7.8 0 0 0 0-2l2.1-1.6-2-3.5-2.5 1a7.6 7.6 0 0 0-1.7-1l-.4-2.6h-4l-.4 2.6a7.6 7.6 0 0 0-1.7 1l-2.5-1-2 3.5L7.6 11a7.8 7.8 0 0 0 0 2l-2.1 1.6 2 3.5 2.5-1c.5.4 1.1.7 1.7 1l.4 2.6h4l.4-2.6c.6-.3 1.2-.6 1.7-1l2.5 1 2-3.5L19.4 13zM12 15.5a3.5 3.5 0 1 1 0-7 3.5 3.5 0 0 1 0 7z',
      lines:'M4 7h10v2H4V7zm0 4h10v2H4v-2zm0 4h10v2H4v-2zm14-9h2v14h-2z'
    };
  ...
  _structSig(m){
    ...
    return [m.salut,m.dateFr,m.palette.color||'',alertSig,btnSig,tileSig,fcSig,this._sheetDay,this._autoPanel,this._setPanel].join('#');
  }
  _patch(m){
  ...
  _render(m){
    ...
    const pastilles=[
      ...
      `<span class="pastille pSet pAuto" data-setpanel="1">${this._ic('cog','pi')}Réglages</span>`
    ].join('');
    ...
    const alertHtml=alerts.length?alerts.map((a,i)=>`
      <div class="alert ${a.cl} ${a.nav?'':'aStatic'}" data-nav="${a.nav||''}" data-ai="${i}">
        ...
        ${a.act==='qveto'?'<button class="aBtn" data-qveto="1">Annuler</button>':a.nav?'<span class="aGo">›</span>':''}
      </div>`).join('')
    ...
    <style>
      ...
      .pSet{background:rgba(255,255,255,.10);color:var(--txt2);}
      .setSheet{max-width:520px;}
      .setList{margin-top:18px;display:flex;flex-direction:column;gap:22px;}
      .setRow{display:flex;flex-direction:column;gap:10px;}
      .setHd{display:flex;align-items:center;justify-content:space-between;gap:12px;}
      .setName{display:inline-flex;align-items:center;gap:8px;font-size:14.5px;font-weight:700;}
      .seti{width:17px;height:17px;flex:none;color:var(--cool);}
      .setVal{font-size:18px;font-weight:800;letter-spacing:-.01em;color:var(--cool);min-width:50px;text-align:right;}
      .setValBm{color:var(--manual);}
      .setSlider{-webkit-appearance:none;appearance:none;width:100%;height:8px;border-radius:999px;background:rgba(255,255,255,.16);outline:none;cursor:pointer;margin:0;}
      .setSlider::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:26px;height:26px;border-radius:999px;background:#fff;border:none;box-shadow:0 2px 7px rgba(0,0,0,.4);cursor:pointer;transition:transform .12s;}
      .setSlider::-webkit-slider-thumb:active{transform:scale(1.14);}
      .setSlider::-moz-range-thumb{width:26px;height:26px;border-radius:999px;background:#fff;border:none;box-shadow:0 2px 7px rgba(0,0,0,.4);cursor:pointer;}
      .setSlider::-moz-range-track{height:8px;border-radius:999px;background:rgba(255,255,255,.16);}
      .setBm::-webkit-slider-thumb{box-shadow:0 2px 7px rgba(0,0,0,.4),0 0 0 2px var(--manual) inset;}
      .setScale{display:flex;justify-content:space-between;font-size:11px;font-weight:700;color:var(--txt2);padding:0 2px;}
      .setHint{font-size:12px;line-height:1.5;color:var(--txt2);background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.09);border-radius:14px;padding:11px 13px;}
      .setHint b{color:#fff;font-weight:800;}
      .czRow{display:flex;flex-direction:column;gap:9px;}
      .czHd{display:flex;align-items:baseline;justify-content:space-between;gap:12px;}
      .czName{font-size:15px;font-weight:800;letter-spacing:-.01em;}
      .czGap{font-size:12.5px;color:var(--txt2);}
      .czTrack{position:relative;height:34px;}
      .czRail{position:absolute;top:14px;left:0;width:100%;height:6px;border-radius:3px;background:rgba(255,255,255,.16);}
      .czHeatFill{position:absolute;top:14px;left:0;height:6px;border-radius:3px 0 0 3px;background:rgba(255,157,111,.6);}
      .czCoolFill{position:absolute;top:14px;height:6px;border-radius:0 3px 3px 0;background:rgba(111,220,255,.6);}
      .czH{position:absolute;top:0;left:0;width:100%;height:34px;margin:0;background:transparent;-webkit-appearance:none;appearance:none;pointer-events:none;}
      .czH::-webkit-slider-runnable-track{background:transparent;border:none;height:34px;}
      .czH::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;pointer-events:auto;width:26px;height:26px;margin-top:4px;border-radius:999px;background:#fff;box-shadow:0 2px 7px rgba(0,0,0,.45);cursor:pointer;border:2px solid #fff;}
      .czHeat::-webkit-slider-thumb{border-color:var(--heat);}
      .czCool::-webkit-slider-thumb{border-color:var(--cool);}
      .czH::-moz-range-track{background:transparent;height:34px;}
      .czH::-moz-range-thumb{pointer-events:auto;width:26px;height:26px;border-radius:999px;background:#fff;box-shadow:0 2px 7px rgba(0,0,0,.45);cursor:pointer;border:2px solid #fff;}
      .czHeat::-moz-range-thumb{border-color:var(--heat);}
      .czCool::-moz-range-thumb{border-color:var(--cool);}
      .czScale{display:flex;justify-content:space-between;font-size:11px;font-weight:700;color:var(--txt2);padding:0 2px;}
      .czVals{display:flex;justify-content:space-between;margin-top:2px;}
      .czVHeat,.czVCool{display:inline-flex;align-items:center;gap:6px;font-size:13px;font-weight:700;}
      .czVHeat{color:var(--heat);}
      .czVCool{color:var(--cool);}
      .czi{width:15px;height:15px;}
      .czCoolBand{position:absolute;top:14px;height:6px;background:rgba(111,220,255,.28);}
      .czCutLine{position:absolute;top:6px;height:22px;border-left:2px dotted var(--cool);transform:translateX(-1px);pointer-events:none;}
      .czVCut{display:inline-flex;align-items:center;gap:4px;font-size:12.5px;font-weight:700;color:var(--cool);opacity:.85;}
      .zmRow{display:flex;align-items:center;justify-content:space-between;gap:12px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.09);border-radius:16px;padding:11px 14px;}
      .zmLeft{display:flex;flex-direction:column;}
      .zmName{font-size:14px;font-weight:800;}
      .zmSub{font-size:11.5px;color:var(--txt2);}
      .zmStep{display:flex;align-items:center;gap:10px;}
      .zmBtn{width:32px;height:32px;border-radius:999px;border:1px solid var(--stroke);background:rgba(255,255,255,.08);color:#fff;font-size:20px;font-weight:700;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:transform .12s,background .15s;}
      .zmBtn:active{transform:scale(.9);background:rgba(255,255,255,.16);}
      .zmVal{min-width:42px;text-align:center;font-size:16px;font-weight:800;letter-spacing:-.01em;}
      .setBmRow{margin-top:4px;padding-top:16px;border-top:1px solid rgba(255,255,255,.10);}
      .adv{margin-top:2px;border:1px solid rgba(255,255,255,.08);border-radius:16px;background:rgba(255,255,255,.03);overflow:hidden;}
      .adv>summary{list-style:none;cursor:pointer;display:flex;align-items:center;gap:9px;padding:13px 15px;font-size:13.5px;font-weight:800;color:var(--txt2);user-select:none;}
      .adv>summary::-webkit-details-marker{display:none;}
      .adv>summary .advi{width:17px;height:17px;color:var(--txt2);flex:none;}
      .adv>summary>span:nth-child(2){flex:1;}
      .advChev{font-size:18px;font-weight:700;color:var(--txt2);transition:transform .2s;line-height:1;}
      .adv[open]>summary .advChev{transform:rotate(90deg);}
      .advBody{display:flex;flex-direction:column;gap:14px;padding:4px 15px 16px;}
      .advBody .zmRow,.advBody .setBmRow{margin:0;}
      .advBody .setBmRow{border-top:none;padding-top:0;}
    ...
    this.shadowRoot.querySelectorAll('[data-setpanel]').forEach(el=>{
      el.addEventListener('click',ev=>{ev.stopPropagation();this._openSet();});
    });
    this.shadowRoot.querySelectorAll('[data-closeset]').forEach(el=>{
      el.addEventListener('click',ev=>{ev.stopPropagation();this._closeSet();});
    });
    {
      const sr=this.shadowRoot;
      const czMn=15,czMx=30;
      const bmMin=parseFloat(this._attr(c.bandeMorte,'min',0.5));
      const zmMin=parseFloat(this._attr(c.zoneMorte,'min',1.5)),zmMax=parseFloat(this._attr(c.zoneMorte,'max',6)),zmStep=parseFloat(this._attr(c.zoneMorte,'step',0.5));
      const pcz=v=>(((v-czMn)/(czMx-czMn))*100);
      const zones=['rdc','etage'];
      const get=(z,kd)=>sr.querySelector(`.czH[data-zone="${z}"][data-kind="${kd}"]`);
      let rebond=this._f(c.bandeMorte,2);
      let gap=Math.max(zmMin,this._f(c.zoneMorte,2));
      const commitHeat=z=>this._call('input_number','set_value',{entity_id:z==='rdc'?c.heatRdc:c.heatEtage,value:parseFloat(get(z,'heat').value)});
      const commitCool=z=>{const v=parseFloat(get(z,'cool').value);if(z==='rdc')this._call('input_number','set_value',{entity_id:c.seuilRdc,value:v});else (Array.isArray(c.seuilEtage)?c.seuilEtage:[c.seuilEtage]).forEach(e=>this._call('input_number','set_value',{entity_id:e,value:v}));};
      const rebondMax=()=>{let m=99;zones.forEach(z=>{const hh=parseFloat(get(z,'heat').value),kk=parseFloat(get(z,'cool').value);m=Math.min(m,kk-hh-1);});return Math.max(bmMin,m);};
      const enforceGap=(z,moved)=>{
        const hi=get(z,'heat'),ki=get(z,'cool');
        let h=parseFloat(hi.value),k=parseFloat(ki.value);
        if(k-h<gap){
          if(moved==='cool'){h=k-gap;if(h<czMn){h=czMn;k=czMn+gap;}}
          else{k=h+gap;if(k>czMx){k=czMx;h=czMx-gap;}}
          hi.value=h;ki.value=k;
        }
      };
      const drawZone=z=>{
        const h=parseFloat(get(z,'heat').value),k=parseFloat(get(z,'cool').value);
        const cut=Math.min(Math.max(k-rebond,h+1),k-0.5);
        const hf=sr.querySelector(`[data-czheatfill="${z}"]`);if(hf)hf.style.width=pcz(h)+'%';
        const bd=sr.querySelector(`[data-czband="${z}"]`);if(bd){bd.style.left=pcz(cut)+'%';bd.style.width=(pcz(k)-pcz(cut))+'%';}
        const cf=sr.querySelector(`[data-czcoolfill="${z}"]`);if(cf){cf.style.left=pcz(k)+'%';cf.style.width=(100-pcz(k))+'%';}
        const ln=sr.querySelector(`[data-czcutline="${z}"]`);if(ln)ln.style.left=pcz(cut)+'%';
        const vh=sr.querySelector(`[data-czvheat="${z}"]`);if(vh)vh.textContent=Math.round(h);
        const vk=sr.querySelector(`[data-czvcool="${z}"]`);if(vk)vk.textContent=Math.round(k);
        const vc=sr.querySelector(`[data-czvcut="${z}"]`);if(vc)vc.textContent=Math.round(cut);
        const g=sr.querySelector(`[data-czgap="${z}"]`);if(g)g.textContent=`neutre ${Math.round(h)}–${Math.round(cut)}°`;
      };
      const drawAll=()=>{rebond=Math.min(Math.max(rebond,bmMin),rebondMax());zones.forEach(drawZone);};
      sr.querySelectorAll('.czH').forEach(inp=>{
        const zone=inp.getAttribute('data-zone');
        const kind=inp.getAttribute('data-kind');
        inp.addEventListener('input',ev=>{
          ev.stopPropagation();
          enforceGap(zone,kind);
          drawAll();
        });
        inp.addEventListener('change',ev=>{
          ...
          enforceGap(zone,kind);
          drawAll();
          if(kind==='heat'){commitHeat(zone);commitCool(zone);}
          else{commitCool(zone);commitHeat(zone);}
          this._call('input_number','set_value',{entity_id:c.bandeMorte,value:rebond});
        });
      });
      const bmS=sr.querySelector('[data-setslider="bm"]');
      if(bmS){
        bmS.addEventListener('input',ev=>{
          ev.stopPropagation();
          rebond=parseFloat(bmS.value);
          drawAll();
          bmS.value=rebond;
          const lab=sr.querySelector('[data-setval="bm"]');if(lab)lab.textContent=rebond.toFixed(1)+'°';
        });
        bmS.addEventListener('change',ev=>{
          ev.stopPropagation();
          rebond=Math.min(Math.max(parseFloat(bmS.value),bmMin),rebondMax());
          drawAll();bmS.value=rebond;
          const lab=sr.querySelector('[data-setval="bm"]');if(lab)lab.textContent=rebond.toFixed(1)+'°';
          this._call('input_number','set_value',{entity_id:c.bandeMorte,value:rebond});
        });
      }
      sr.querySelectorAll('[data-zm]').forEach(b=>{
        b.addEventListener('click',ev=>{
          ev.stopPropagation();
          const dir=b.getAttribute('data-zm')==='+'?1:-1;
          gap=Math.min(zmMax,Math.max(zmMin,Math.round((gap+dir*zmStep)*2)/2));
          const zl=sr.querySelector('[data-zmval]');if(zl)zl.textContent=gap+'°';
          zones.forEach(z=>enforceGap(z,'gap'));
          drawAll();
          this._call('input_number','set_value',{entity_id:c.zoneMorte,value:gap});
          zones.forEach(z=>{commitCool(z);commitHeat(z);});
        });
      });
    }
