...
.secTitle{font-size:20px;margin:6px 4px 14px}
  ...
  _sheetHtml(){const r=this._c.rooms.find(x=>x.key===this._open);
    ...
    else if(cool){dval=cons;dlab=`D\u00e9marre \u00e0 ${cons}\u00b0 \u00b7 s'arr\u00eate \u00e0 ${cutv}\u00b0`;steps=true;}
    ...
    <div class='sheet open'><div class='grab'></div>
      ...
      <div class='dial'>
        ${steps?`<button class='stepBtn' data-act='step' data-k='${r.key}' data-d='-0.5'>\u2212</button>`:''}
  ...
  _setSheetHtml(){const c=this._c;
    ...
    <div class='sheet open sheetScroll'><div class='grab'></div>
      ...
      ${sh('#6fdcff','Froid automatique',icSnow)}
      ...
      ${sh('#79e3c0','D\u00e9shumidification automatique',icDrop)}
      ...
      ${sh('var(--manual)','Allumage manuel',icPow)}
  ...
  _click(e){const t=e.target.closest('[data-act]');if(!t)return;
    ...
    if(act==='nstep'){const en=t.dataset.e;const st=parseFloat(this._a(en,'step'))||1;const mn=parseFloat(this._a(en,'min'));const mx=parseFloat(this._a(en,'max'));let v=parseFloat(this._s(en))+st*parseInt(t.dataset.d);v=Math.min(mx,Math.max(mn,v));h.callService('input_number','set_value',{entity_id:en,value:v});return;}
